...
bool ipSetFlag = false;
bool gatewaySetFlag = false;
bool routerMacAddressSetFlag = false;
...
bool gatewayIsSet() {
    ...
    if(total != 0)
        gatewaySetFlag = true;
    return gatewaySetFlag;
}
bool packetIsAddressedToMe(etherHeader *ether) {
    ipHeader *ip = (ipHeader*)ether->data;
    ...
    {
        ok = (ether->destAddress[i] == hwAddress[i]);
        i++;
    }
    if(ok)
        ok = htons(tcp->destPort) == SOURCEPORT;
    if(ok)
        tcpExtractData(ether);
    return ok;
}
void getRouterMacAddress(uint8_t mac[6]) {
    uint8_t i = 0;
    for(i = 0; i < HW_ADD_LENGTH; i++) {
        mac[i] = routerMacAddress[i];
    }
}
bool routerMacAddressIsSet() {
    uint8_t mac[6];
    getRouterMacAddress(mac);
    uint8_t i;
    uint32_t total = 0;
    for(i = 0; i < HW_ADD_LENGTH; i++) {
        total += mac[i];
    ...
    if(total != 0)
        routerMacAddressSetFlag = true;
    return routerMacAddressSetFlag;
}
void setRouterMacAddress(uint8_t mac[6]) {
    uint8_t i;
    for(i = 0; i < HW_ADD_LENGTH; i++) {
        routerMacAddress[i] = mac[i];
    }
}
void sendArpRequestToRouter(etherHeader *ether) {
    uint8_t myIp[4];
    uint8_t routerIp[4];
    etherGetIpAddress(myIp);
    etherGetIpGatewayAddress(routerIp);
    etherSendArpRequest(ether, myIp, routerIp);
}
