// Ethernet Example
// Happy Ndikumana and Jason Losh
...
//                changeTcpConnectFlagState(false);
            ...
            if (strcmp(token, "connect") == 0) {
                changeTcpConnectFlagState(true);
            }
            ...
            if (strcmp(token, "help") == 0)
            {
                ...
                putsUart0("  dhcp on, off, renew, release, rebind\r");
...
int main(void)
{
    ...
    etherSetMacAddress(2, 3, 4, 5, 6, 107);
...
//    while(1);
    ...
    while (true)
    {
        ...
        if(((ipIsSet() && gatewayIsSet()) || ((dhcpGetState() == DHCP_BOUND) && ipIsSet() && gatewayIsSet())) && !routerMacAddressIsSet()) {
            sendArpRequestToRouter(data);
        }
        if(((ipIsSet() && gatewayIsSet()) && tcpConnect() || ((dhcpGetState() == DHCP_BOUND) && ipIsSet() && gatewayIsSet())) && routerMacAddressIsSet()) {
            tcpClientSendPendingMessages(data);
        }
        if((ipIsSet() && gatewayIsSet())|| dhcpIsEnabled()) {
        // Packet processing
            if (etherIsDataAvailable())
            {
                ...
                if (etherIsArpResponse(data)) {
                    ...
                    else if(dhcpGetState() == DHCP_BOUND && !routerMacAddressIsSet()){
                        tcpProcessArpResponse(data);
                    }
    ...
    //                        etherSendUdpResponse(data, (uint8_t*)"Received", 9);
                        ...
                        if(etherIsTcp(data) && packetIsAddressedToMe(data)) {
                            //process TCP responses
                            tcpProcessTcpResponse(data);
                        }
                    }
    //            	else
    //            	{
